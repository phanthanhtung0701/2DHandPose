...
from HAND_model.utils.options import parse_args_function
from HAND_model.utils.dataset import Dataset
from HAND_model.utils.custom_data import Custom_Dataset
...
if args.train:
    trainset = Custom_Dataset(root=root, load_set='train', transform=transform)
...
if args.val or args.train:
    valset = Custom_Dataset(root=root, load_set='val', transform=transform)
...
if args.test:
    testset = Custom_Dataset(root=root, load_set='test', transform=transform)
...
if args.train:
    ...
    for epoch in range(start, args.num_iterations):  # loop over the dataset multiple times
        ...
        for i, tr_data in enumerate(trainloader):
            # get the inputs
            inputs, hand_bb, point2d, labels2d = tr_data

            # wrap them in Variable
            inputs = Variable(inputs)
            hand_bb = Variable(hand_bb)
            point2d = Variable(point2d)
            labels2d = Variable(labels2d)

            if use_cuda and torch.cuda.is_available():
                inputs = inputs.float().cuda(device=args.gpu_number[0])
                hand_bb = hand_bb.float().cuda(device=args.gpu_number[0])
                point2d = point2d.float().cuda(device=args.gpu_number[0])
                labels2d = labels2d.float().cuda(device=args.gpu_number[0])

            # zero the parameter gradients
            optimizer.zero_grad()

            outputs2d = model(inputs, point2d)
        ...
        if args.val and (epoch + 1) % args.val_epoch == 0:
            ...
            for v, val_data in enumerate(valloader):
                # get the inputs
                inputs, hand_bb, point2d, labels2d = val_data
...
if args.test:
    ...
    for i, ts_data in enumerate(testloader):
        # get the inputs
        inputs, hand_bb, point2d, labels2d = ts_data
        ...
        outputs2d = model(inputs, point2d)
        loss2d_init = criterion(point2d, labels2d)
        loss2d = criterion(outputs2d, labels2d)
        print(f"loss:  {loss2d_init}    {loss2d}")
        loss = loss2d
        running_loss += loss.data
        running_init_loss += loss2d_init.data

    print(i)
    print('init test error: %.5f' % (running_init_loss / (i + 1)))
